...
class User:
    ...
    def save(cls, formulario):
        ...
        result = connectToMySQL('recipes').query_db(query, formulario) #me regresan el nuevo ID de la persona registrada
        #result = 5
        return result
    ...
    def valida_usuario(formulario): #El formulario es 
        ...
        results = connectToMySQL('recipes').query_db(query, formulario)
        if len(results) >= 1:
    ...
    def get_by_email(cls, formulario):
        ...
        #}
        query = "SELECT * FROM users WHERE email = %(email)s"
        result = connectToMySQL('recipes').query_db(query, formulario) #Los SELECT regresan una lista
    ...
    def get_by_id(cls, formulario):
        ...
        result = connectToMySQL('recipes').query_db(query, formulario) #RECIBIMOS UNA LISTA
